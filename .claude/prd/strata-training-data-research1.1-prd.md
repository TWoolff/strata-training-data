...
|--------|---------|--------|---------------|
| Training data style diversity | 1 source (Mixamo 3D renders) | 4+ sources (Mixamo + Live2D + VRoid + PSD) | Count distinct art styles in training set |
...
| Measurement extraction accuracy | N/A | Mean error ≤ 5% of true dimension | Compare 2D-extracted vs. mesh ground truth on held-out characters |
| Multi-view angle coverage | 1 angle (front) | 5 angles (0°, 45°, 90°, 135°, 180°) | Verify all angles present in training output |
| VRoid training data volume | 0 | 10,000+ multi-angle renders | Count in output/segmentation/vroid/ |
...
### 13.8 Updated Repository Structure (3D Mesh Additions)

```
strata-training-data/
├── pipeline/
│   ├── ...existing...
│   ├── multi_angle_renderer.py         ← NEW — render from multiple camera angles
│   ├── vroid_importer.py               ← NEW — import VRM/VRoid models into Blender
│   ├── measurement_extractor.py        ← NEW — extract per-label 2D→3D measurements
│   ├── measurement_ground_truth.py     ← NEW — true body dimensions from mesh
│   ├── contour_renderer.py             ← NEW — Freestyle contour on/off pairs
│   └── contour_augmenter.py            ← NEW — hand-drawn contour style variations
│
├── data/
│   ├── ...existing...
│   └── vroid/                          ← ⛔ .gitignore — NEW
│       └── README.md                   ← VRoid Hub sources, PAniC-3D methodology
│
├── segmentation/
│   ├── scripts/
│   │   ├── ...existing...
│   │   └── vroid_mapper.py             ← NEW — VRoid material/bone → Strata label
│   └── labels/
│       ├── live2d_mappings.csv
│       └── vroid_mappings.csv          ← NEW — VRoid-specific label overrides
│
├── mesh/                               ← NEW directory
│   ├── scripts/
│   │   ├── texture_projection_trainer.py  ← NEW (future) — partial→complete texture pairs
│   │   └── proportion_clusterer.py     ← NEW — cluster measurement profiles → template archetypes
│   └── measurements/
│       └── measurement_profiles.json   ← NEW — per-character ground truth dimensions
│
├── ...rest unchanged...
```

### 13.9 Implementation Priority (3D Mesh Data)

**Phase 1 — With Existing Pipeline Work (weeks 1–3)**

| Task | Effort | Impact | Why Now |
|------|--------|--------|---------|
| multi_angle_renderer.py — add camera angles to Mixamo pipeline | 2 days | Critical | Segmentation model WILL fail on 3/4 views without this. Must be in first training run. |
...
| proportion_clusterer.py — find natural template archetypes | 1 day | High | Determines how many templates to build and what proportions they should have. |

**Phase 2 — VRoid Integration (weeks 4–8)**

| Task | Effort | Impact | Why Now |
|------|--------|--------|---------|
| VRoid model acquisition (2,000+ models) | 1 week | High | Anime-style multi-view data. Every competitor trains on this. |
| vroid_importer.py + vroid_mapper.py | 3 days | High | Import pipeline + label mapping. |
| Multi-angle VRoid renders | 2 days (compute) | High | 2,000 chars × 5 angles = 10,000 anime-style training images with perfect ground truth. |
| contour_renderer.py + augmenter | 3 days | Medium | Enables contour removal for texture projection. |

**Phase 3 — Measurement Validation (weeks 6–10)**

| Task | Effort | Impact | Why Now |
|------|--------|--------|---------|
| measurement_extractor.py — 2D→3D extraction pipeline | 3 days | High | Core of the template deformation pipeline. Needs ground truth to validate. |
...
### 13.10 Volume Summary (Updated)

| Data Source | Views | Volume | Style |
|-------------|-------|--------|-------|
| Mixamo renders (existing) | Front only | ~2,000 images | Realistic/western 3D |
| Mixamo renders (extended) | 5 angles | ~10,000 images | Realistic/western 3D, multi-view |
| Live2D models | Front only | ~1,600 images | Anime/stylized 2D illustration |
| VRoid renders (new) | 5 angles | ~10,000–25,000 images | Anime 3D, multi-view |
| PSD extraction | Front only | ~50–100 images | Mixed hand-drawn styles |
| Contour pairs | 5 angles × 5 styles | ~50,000 pairs | Contour/no-contour variants |
| **Total segmentation training** | | **~22,000–37,000 images** | **3 sources, 5+ art styles, multi-view** |
