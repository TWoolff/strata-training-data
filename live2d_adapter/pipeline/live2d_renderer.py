...
# ---------------------------------------------------------------------------
# Node.js / Cubism SDK renderer
# ---------------------------------------------------------------------------

# Path to the render_live2d.js script (relative to this file's package).
_NODE_RENDERER_DIR = Path(__file__).parent.parent / "tools" / "live2d_renderer"
_NODE_RENDERER_JS = _NODE_RENDERER_DIR / "render_live2d.js"

# Fallback: /tmp/live2d_renderer (created during development)
_NODE_RENDERER_FALLBACK = Path("/tmp/live2d_renderer/render_live2d.js")


def _find_node_renderer() -> Path | None:
    """Return path to render_live2d.js, or None if not found."""
    if _NODE_RENDERER_JS.exists():
        return _NODE_RENDERER_JS
    if _NODE_RENDERER_FALLBACK.exists():
        return _NODE_RENDERER_FALLBACK
    return None
...
def _render_via_node(
...
) -> tuple[Image.Image, dict, Image.Image | None] | None:
    """Render a Live2D model via the Node.js Cubism SDK renderer.

    Args:
        model_dir: Directory containing the .model3.json and .moc3 files.
        resolution: Output resolution (square).
        seg_map: Optional dict of mesh_id → region_id. If provided, the JS
            renderer runs a WebGL segmentation pass and returns a seg image.

    Returns:
        Tuple of (composite RGBA image, meta dict, seg image or None) or None on failure.
        meta dict has keys: drawableMeta, renderTransform.
        seg image is an 8-bit grayscale PIL Image with pixel = region_id (only
        when seg_map is provided and the JS seg pass succeeds).
    """
    node_js = _find_node_renderer()
    if node_js is None:
        logger.error("Node.js renderer not found. Expected at %s or %s", _NODE_RENDERER_JS, _NODE_RENDERER_FALLBACK)
        return None
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cmd = [node_bin, str(node_js), str(model_dir), str(tmp_png), str(resolution)]

        if seg_map is not None:
            tmp_seg_map = Path(tmpdir) / "seg_map.json"
            tmp_seg_map.write_text(json.dumps(seg_map))
            cmd.append(str(tmp_seg_map))
        ...
        try:
            result = subprocess.run(
                ...
                text=True,
                timeout=120,
                cwd=str(node_js.parent),
            )
        except subprocess.TimeoutExpired:
            logger.error("Node.js renderer timed out for %s", model_dir.name)
            return None
        except Exception as exc:
            logger.error("Node.js renderer error for %s: %s", model_dir.name, exc)
            return None

        if result.returncode != 0:
            stderr = result.stderr.strip()
            logger.error("Node.js renderer failed for %s: %s", model_dir.name, stderr[:500])
            return None

        # Log stdout (canvas info)
        for line in result.stdout.strip().splitlines():
            logger.debug("node: %s", line)

        if not tmp_png.exists():
            logger.error("Node.js renderer produced no output for %s", model_dir.name)
            return None

        # Load composite image
        try:
            composite = Image.open(tmp_png).convert("RGBA")
        except Exception as exc:
            logger.error("Failed to load rendered PNG for %s: %s", model_dir.name, exc)
            return None

        # Load meta JSON
        meta_path = Path(str(tmp_png).replace(".png", ".meta.json"))
        meta: dict = {}
        if meta_path.exists():
            try:
                meta = json.loads(meta_path.read_text())
            except Exception as exc:
                logger.warning("Failed to load meta JSON for %s: %s", model_dir.name, exc)

        # Load seg image if the JS renderer produced one
        seg_img: Image.Image | None = None
        seg_path = Path(str(tmp_png).replace(".png", ".seg.png"))
        if seg_path.exists():
            try:
                # The JS shader encodes: pixel_value = round(region_id * 10)
                # so region 1 → 10, region 21 → 210, background → 0.
                # Decode: region_id = round(pixel_value / 10).
                seg_rgba = Image.open(seg_path).convert("RGBA")
                seg_arr = np.array(seg_rgba)
                red = seg_arr[:, :, 0].astype(np.float32)
                alpha = seg_arr[:, :, 3]
                # Nearest-integer decode: round(red / 10)
                decoded = np.round(red / 10.0).astype(np.uint8)
                mask_arr = np.where(alpha > 0, decoded, 0).astype(np.uint8)
                seg_img = Image.fromarray(mask_arr, mode="L")
            except Exception as exc:
                logger.warning("Failed to load seg PNG for %s: %s", model_dir.name, exc)

        return composite, meta, seg_img


def _build_segmentation_from_meta(
    meta: dict,
    fragment_to_region: dict[str, int],
    resolution: int = RENDER_RESOLUTION,
) -> tuple[np.ndarray, np.ndarray]:
    """Build segmentation mask and draw order map from Node.js renderer meta.

    Rasterizes each visible ArtMesh triangle with its region color using the
    same transform (centerX, centerY, scale) that the Node.js WebGL renderer used.

    Args:
        meta: Meta dict from _render_via_node (drawableMeta + renderTransform).
        fragment_to_region: Mesh ID → region ID mapping.
        resolution: Output resolution.

    Returns:
        Tuple of (segmentation mask, draw order map) as uint8 numpy arrays.
    """
    mask = np.zeros((resolution, resolution), dtype=np.uint8)
    draw_order_map = np.zeros((resolution, resolution), dtype=np.uint8)

    transform = meta.get("renderTransform", {})
    crop_x = transform.get("cropX", 0.0)
    crop_y = transform.get("cropY", 0.0)
    crop_scale = transform.get("scale", 1.0)
    dx = transform.get("dx", 0.0)
    dy = transform.get("dy", 0.0)

    # New transform format (pixi-live2d-display renderer):
    #   model_scale, model_x, model_y from pixi placement
    # Old format (Core-only renderer, kept for backward compat):
    #   mnX, mxX, mnY, mxY, renderSize
    if "modelScale" in transform:
        # Full Cubism → pixi → output pixel transform:
        #   1. Cubism model units → canvas pixels:
        #      cx = pos_x * ppu + originX
        #      cy = -pos_y * ppu + originY  (Y-flip: Cubism Y-up → canvas Y-down)
        #   2. Canvas pixels → pixi screen pixels:
        #      px_pixi = cx * modelScale + modelX
        #      py_pixi = cy * modelScale + modelY
        #   3. Pixi screen pixels → output pixel (via crop):
        #      px_out = (px_pixi - cropX) * cropScale + dx
        #      py_out = (py_pixi - cropY) * cropScale + dy
        model_sc = transform["modelScale"]
        model_x = transform.get("modelX", 0.0)
        model_y = transform.get("modelY", 0.0)
        ppu = transform.get("pixelsPerUnit", 1.0)
        origin_x = transform.get("canvasOriginX", 0.0)
        origin_y = transform.get("canvasOriginY", 0.0)

        def model_to_screen(mx_val: float, my_val: float) -> tuple[float, float]:
            cx = mx_val * ppu + origin_x
            cy = -my_val * ppu + origin_y
            px_pixi = cx * model_sc + model_x
            py_pixi = cy * model_sc + model_y
            px_out = (px_pixi - crop_x) * crop_scale + dx
            py_out = (py_pixi - crop_y) * crop_scale + dy
            return px_out, py_out
    else:
        # Old format: model space → intermediate canvas pixel → output pixel
        render_size = transform.get("renderSize", resolution)
        mn_x = transform.get("mnX", -1.0)
        mx_x = transform.get("mxX", 1.0)
        mn_y = transform.get("mnY", -1.0)
        mx_y = transform.get("mxY", 1.0)
        model_w = max(mx_x - mn_x, 1e-6)
        model_h = max(mx_y - mn_y, 1e-6)

        def model_to_screen(mx_val: float, my_val: float) -> tuple[float, float]:
            px_mid = (mx_val - mn_x) / model_w * render_size
            py_mid = (mx_y - my_val) / model_h * render_size
            px_out = (px_mid - crop_x) * crop_scale + dx
            py_out = (py_mid - crop_y) * crop_scale + dy
            return px_out, py_out

    drawables = meta.get("drawableMeta", [])

    # Compute render order normalization
    visible = [d for d in drawables if d.get("visible") and d.get("opacity", 0) > 0 and "positions" in d]
    if not visible:
        return mask, draw_order_map

    # drawOrder is the actual layer depth from the model file (e.g. 200, 500, 750).
    # Fall back to renderOrder for older meta JSONs that lack drawOrder.
    draw_orders = [d.get("drawOrder", d.get("renderOrder", 0)) for d in visible]
    min_order = min(draw_orders)
    max_order = max(draw_orders)
    order_range = max(max_order - min_order, 1)

    # Sort by draw order (back to front)
    sorted_vis = sorted(visible, key=lambda d: d.get("drawOrder", d.get("renderOrder", 0)))

    for drawable in sorted_vis:
        mesh_id = drawable["id"]
        region_id = fragment_to_region.get(mesh_id, 0)
        if region_id <= 0:
            continue
        ...
        raw_order = drawable.get("drawOrder", drawable.get("renderOrder", 0))
        norm_depth = int((raw_order - min_order) / order_range * 255)
        ...
        for t in range(n_tris):
            ...
            sx0, sy0 = model_to_screen(positions[i0 * 2], positions[i0 * 2 + 1])
            sx1, sy1 = model_to_screen(positions[i1 * 2], positions[i1 * 2 + 1])
            sx2, sy2 = model_to_screen(positions[i2 * 2], positions[i2 * 2 + 1])

            pts = np.array([[sx0, sy0], [sx1, sy1], [sx2, sy2]], dtype=np.float32)
            pts_int = pts.astype(np.int32)

            # Fill triangle with region ID
            cv2.fillPoly(mask, [pts_int], region_id)
            cv2.fillPoly(draw_order_map, [pts_int], norm_depth)
...
def _find_model_json(model_dir: Path) -> Path | None:
    ...
    """
    candidates = sorted(p for p in model_dir.glob("*.model3.json") if not p.name.startswith("."))
    if candidates:
        return candidates[0]
    # Fallback: look for any .json that contains "FileReferences"
    for json_path in sorted(p for p in model_dir.glob("*.json") if not p.name.startswith(".")):
        try:
...
def _load_cdi_json(model_dir: Path) -> dict[str, str]:
    ...
    """
    cdi_files = sorted(p for p in model_dir.glob("*.cdi3.json") if not p.name.startswith("."))
    if not cdi_files:
        return {}
...
def _discover_fragment_images(model_dir: Path) -> list[tuple[str, Path]]:
    ...
    # Search known subdirectories and the model root itself.
    # Model-root PNGs are included but large atlases (≥1024px) are filtered out.
    candidate_dirs = [
        model_dir / "parts",
        model_dir / "images",
        model_dir,
    ...
    for search_dir in candidate_dirs:
        if not search_dir.is_dir():
            continue
        for png_path in sorted(p for p in search_dir.glob("*.png") if not p.name.startswith(".")):
            if png_path in seen:
                continue
            seen.add(png_path)
            # Skip texture atlas naming patterns
            stem = png_path.stem.lower()
            if stem.startswith("texture") or stem.startswith("tex_") or stem.startswith("atlas"):
                continue
            # Skip large images — atlases are typically ≥1024px
            try:
                ...
                continue
            fragments.append((png_path.stem, png_path))
...
def _find_moc3(model_dir: Path) -> Path | None:
    ...
    candidates = sorted(p for p in model_dir.glob("*.moc3") if not p.name.startswith("."))
...
def process_live2d_model(
    ...
    if not fragment_images_raw:
        # Try Node.js / Cubism SDK rendering path (preferred — uses correct vertex positions)
        node_renderer = _find_node_renderer()
        if node_renderer is not None and _find_model_json(model_dir) is not None:
            logger.info("Using Node.js Cubism SDK renderer for model %s", model_name)

            # Pass 1: render without seg_map to discover drawable IDs
            node_result = _render_via_node(model_dir, resolution)
            if node_result is not None:
                composite_img, meta, _ = node_result

                # Build region mapping from drawable IDs in meta.
                # Priority: CDI display name → partId → mesh ID → background
                drawables_meta = meta.get("drawableMeta", [])
                fragment_to_region: dict[str, RegionId] = {}
                unmapped: list[str] = []

                for d in drawables_meta:
                    mesh_id = d["id"]
                    part_id = d.get("partId", "")

                    # 1. Try CDI display name for the mesh ID
                    display_name = cdi_names.get(mesh_id, "")
                    if not display_name and part_id:
                        # 2. Try CDI display name for the part ID
                        display_name = cdi_names.get(part_id, "")
                    if display_name:
                        _, region_id = map_fragment(display_name)
                    else:
                        region_id = -1

                    # 3. Try mapping the mesh ID directly (handles named models)
                    if region_id < 0:
                        _, region_id = map_fragment(mesh_id)

                    # 4. Try mapping the part ID (handles ArtMesh* models with semantic parts).
                    #    Also applies when the mesh ID is a generic placeholder (ArtMesh\d+,
                    #    D_PSD_\d+, etc.) that the exclusion rule tagged as background (rid=0).
                    #    Strip common "Part" prefix so "PartEye" → "Eye", "PartArmB" → "ArmB".
                    _mesh_is_generic = bool(re.match(r"^(?:artmesh|d_psd|d_mesh)\d*$", mesh_id, re.IGNORECASE))
                    if (region_id < 0 or (region_id == 0 and _mesh_is_generic)) and part_id:
                        _pid_rid: int = -1
                        _, _pid_rid = map_fragment(part_id)
                        if _pid_rid < 0:
                            stripped = re.sub(r"^Part", "", part_id)
                            if stripped != part_id:
                                _, _pid_rid = map_fragment(stripped)
                        if _pid_rid >= 0:
                            region_id = _pid_rid

                    if region_id >= 0:
                        fragment_to_region[mesh_id] = region_id
                    else:
                        fragment_to_region[mesh_id] = 0
                        if d.get("visible") and d.get("opacity", 0) > 0:
                            unmapped.append(mesh_id)

                mapped_count = sum(1 for v in fragment_to_region.values() if v > 0)
                total_count = len([d for d in drawables_meta if d.get("visible") and d.get("opacity", 0) > 0 and "positions" in d])

                logger.info(
                    "Live2D Node.js mapping for %s: %d/%d visible drawables mapped",
                    model_name, mapped_count, total_count,
                )
                if unmapped:
                    logger.debug("Unmapped in %s: %s", model_name, unmapped[:20])

                # Pass 2: re-render with seg_map so JS WebGL produces a correct seg image
                seg_map = {mid: int(rid) for mid, rid in fragment_to_region.items() if rid > 0}
                node_result2 = _render_via_node(model_dir, resolution, seg_map=seg_map)
                if node_result2 is not None:
                    composite_img, meta, seg_pil = node_result2
                else:
                    seg_pil = None

                # Use JS-rendered seg image if available; fall back to Python rasterization
                composite_arr = np.array(composite_img)
                alpha = composite_arr[:, :, 3]
                if seg_pil is not None:
                    mask = np.array(seg_pil.convert("L"))
                    mask[alpha == 0] = 0
                    # Build draw order from meta geometry (still needed — JS doesn't output it)
                    _, draw_order_map_arr = _build_segmentation_from_meta(
                        meta, fragment_to_region, resolution
                    )
                    draw_order_map_arr[alpha == 0] = 0
                else:
                    # Fallback: Python rasterization (less accurate)
                    mask, draw_order_map_arr = _build_segmentation_from_meta(
                        meta, fragment_to_region, resolution
                    )
                    mask[alpha == 0] = 0
                ...
                return Live2DRenderResult(
                    ...
                    image=composite_img,
                    mask=mask,
                    draw_order_map=draw_order_map_arr,
                    joint_data=joint_data,
                    fragment_count=total_count,
                    mapped_count=mapped_count,
                    unmapped_fragments=unmapped[:50],
                )

        # Fallback: old .moc3 binary extraction path (deprecated — produces wrong images)
        if model and model.texture_paths:
            ...
            if textures:
                ...
                if moc3_fragments:
                    ...
                    fragment_to_region2: dict[str, RegionId] = {}
                    unmapped2: list[str] = []
                    loaded_fragments: list[tuple[str, Image.Image, int]] = []
                    ...
                    for frag_name, frag_img, draw_order in moc3_fragments:
                        ...
                        if region_id < 0:
                            # Strip common "Part" prefix: "PartHead" → "Head" → head(1)
                            stripped = re.sub(r"^Part", "", frag_name)
                            if stripped != frag_name:
                                _, region_id = map_fragment(stripped)
                        if region_id >= 0:
                            fragment_to_region2[frag_name] = region_id
                        else:
                            fragment_to_region2[frag_name] = 0
                            unmapped2.append(frag_name)
...
def process_live2d_directory(
    ...
    *,
    ...
    model_dirs: list[Path] = sorted(p for p in live2d_dir.iterdir() if p.is_dir() and not p.name.startswith("."))
