"""Train the diffusion-enhanced weight prediction model (Model 4).

CLI entry point::

    python -m training.train_diffusion_weights \\
        --config training/configs/diffusion_weights.yaml
    python -m training.train_diffusion_weights \\
        --config training/configs/diffusion_weights.yaml \\
        --resume checkpoints/diffusion_weights/best.pt

Trains on per-vertex features + precomputed segmentation encoder features
with combined KL divergence (soft weight labels) + BCE (confidence) loss.
"""
...
from training.data.diffusion_weight_dataset import (
    DiffusionWeightDataset,
    DiffusionWeightDatasetConfig,
)
from training.models.diffusion_weight_model import DiffusionWeightPredictionModel
from training.train_weights import adjust_lr, compute_loss
...
def collate_fn(batch: list[dict]) -> dict[str, torch.Tensor]:
    ...
    return {
        ...
        "diffusion_features": torch.stack([b["diffusion_features"] for b in batch]),
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        diff_features = batch["diffusion_features"].to(device)
        targets = {
            k: v.to(device) for k, v in batch.items() if k not in ("features", "diffusion_features")
        }

        outputs = model(features, diff_features)
...
def validate(
    ...
    for batch in loader:
        ...
        pred_weights_logits = outputs["weights"].squeeze(-1).cpu()
        pred_weights = F.softmax(pred_weights_logits, dim=1).numpy()
        gt_weights = batch["weights_target"].numpy()

        pred_conf = torch.sigmoid(outputs["confidence"].squeeze(-1).squeeze(1)) > 0.5
        pred_conf = pred_conf.cpu().numpy()
        gt_conf = batch["confidence_target"].numpy()
        num_verts = batch["num_vertices"].numpy()
...
def train(config: dict, resume_path: str | None = None) -> None:
    ...
    # ---- Datasets ----
    dataset_dirs = [Path(d) for d in data_cfg.get("dataset_dirs", [])]
    encoder_features_dirs = [Path(d) for d in data_cfg.get("encoder_features_dirs", [])]
    ds_config = DiffusionWeightDatasetConfig.from_dict(config)

    train_dataset = DiffusionWeightDataset(
        dataset_dirs, encoder_features_dirs, split="train", config=ds_config
    )
    val_dataset = DiffusionWeightDataset(
        dataset_dirs, encoder_features_dirs, split="val", config=ds_config
    )

    if len(train_dataset) == 0:
        logger.error("Training dataset is empty. Check dataset_dirs and encoder_features_dirs.")
        return
    ...
    encoder_channels = model_cfg.get("encoder_channels", 960)
    model = DiffusionWeightPredictionModel(
    ...
    # ---- TensorBoard ----
    save_dir = Path(ckpt_cfg.get("save_dir", "./checkpoints/diffusion_weights"))
    ...
    logger.info(
        "Starting training: %d epochs, batch_size=%d, lr=%.1e, encoder_channels=%d, device=%s",
        epochs,
        batch_size,
        lr,
        encoder_channels,
...
def main() -> None:
    """CLI entry point for diffusion weight prediction training."""
    parser = argparse.ArgumentParser(
        description="Train Strata diffusion-enhanced weight prediction model"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="training/configs/diffusion_weights.yaml",
