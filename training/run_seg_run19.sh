#!/usr/bin/env bash
# =============================================================================
# Strata Training — Run 19 Seg (A100)
#
# Class 20 "unused" remapped to background + Dr. Li's new labels (when ready).
...
#
# Usage:
#   chmod +x training/run_seg_run19.sh
#   ./training/run_seg_run19.sh
# =============================================================================
set -euo pipefail

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_DIR="./logs/run19_seg_${TIMESTAMP}"
mkdir -p "$LOG_DIR"

echo "============================================"
echo "  Strata Training — Run 19 Seg"
echo "  Started: $(date)"
echo "  Logs: $LOG_DIR"
echo "============================================"
echo ""
...
# ---------------------------------------------------------------------------
# 0. Download run 18 checkpoint + frozen splits
# ---------------------------------------------------------------------------
echo "[0/5] Downloading checkpoint + frozen splits..."
mkdir -p checkpoints/segmentation

RUN18_CKPT="checkpoints/segmentation/run18_best.pt"
if [ -f "$RUN18_CKPT" ]; then
    echo "  run18_best.pt already exists."
else
    echo "  Downloading run 18 checkpoint..."
    rclone copy hetzner:strata-training-data/checkpoints_run18_seg/segmentation/run18_best.pt \
        ./checkpoints/segmentation/ --transfers 32 --fast-list -P
    if [ ! -f "$RUN18_CKPT" ]; then
        echo "  FATAL: Could not download run18_best.pt"
        exit 1
    fi
fi
...
echo "  Resuming from: $RUN18_CKPT (run 18 best)"
echo "  Config: training/configs/segmentation_a100_run19.yaml"
echo "  Frozen splits: $FROZEN_FILE"
echo "  Class 20 remap: unused → background (in dataset loader)"
echo ""

python -m training.train_segmentation \
    --config training/configs/segmentation_a100_run19.yaml \
    --reset-epochs \
    --resume "$RUN18_CKPT" \
...
if [ -f "checkpoints/segmentation/best.pt" ]; then
    cp checkpoints/segmentation/best.pt checkpoints/segmentation/run19_best.pt
    python -m training.export_onnx \
        --model segmentation \
        --checkpoint checkpoints/segmentation/run19_best.pt \
        --output ./models/onnx/segmentation_run19.onnx \
        2>&1 | tee "$LOG_DIR/export.log"
    echo "  Exported segmentation_run19.onnx"
fi

rclone copy ./checkpoints/segmentation/run19_best.pt \
    hetzner:strata-training-data/checkpoints_run19_seg/segmentation/ \
    --transfers 32 --fast-list -P
rclone copy "$LOG_DIR/" hetzner:strata-training-data/logs/run19_seg_${TIMESTAMP}/ \
    --transfers 32 --fast-list -P
if [ -f "./models/onnx/segmentation_run19.onnx" ]; then
    rclone copy ./models/onnx/segmentation_run19.onnx \
        hetzner:strata-training-data/models/onnx_run19_seg/ \
        --transfers 32 --fast-list -P
fi

echo ""
echo "============================================"
echo "  Run 19 Seg complete!"
echo "  Finished: $(date)"
echo "  Results:"
grep -E "New best mIoU|mIoU=" "$LOG_DIR/segmentation.log" 2>/dev/null | tail -5 || echo "  (check logs)"
echo ""
echo "  To download to Mac:"
echo "    rclone copy hetzner:strata-training-data/checkpoints_run19_seg/ /Volumes/TAMWoolff/data/checkpoints_run19_seg/ --transfers 32 --fast-list -P"
echo "    rclone copy hetzner:strata-training-data/models/onnx_run19_seg/ /Volumes/TAMWoolff/data/models/onnx_run19_seg/ --transfers 32 --fast-list -P"
echo "============================================"
