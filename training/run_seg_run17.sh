...
#
# Usage:
#   chmod +x training/run_seg_run17.sh
#   ./training/run_seg_run17.sh
# =============================================================================
set -euo pipefail

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_DIR="./logs/run17_seg_${TIMESTAMP}"
mkdir -p "$LOG_DIR"

echo "============================================"
echo "  Strata Training — Run 17 Seg"
echo "  Started: $(date)"
echo "  Logs: $LOG_DIR"
echo "============================================"
echo ""
...
# ---------------------------------------------------------------------------
# 0. Download run 16 checkpoint
# ---------------------------------------------------------------------------
echo "[0/5] Downloading checkpoint..."
mkdir -p checkpoints/segmentation

RUN16_CKPT="checkpoints/segmentation/run16_best.pt"
if [ -f "$RUN16_CKPT" ]; then
    echo "  run16_best.pt already exists."
else
    echo "  Downloading run 16 checkpoint..."
    rclone copy hetzner:strata-training-data/checkpoints_run16_seg/segmentation/run16_best.pt \
        ./checkpoints/segmentation/ --transfers 32 --fast-list -P
    if [ ! -f "$RUN16_CKPT" ]; then
        echo "  FATAL: Could not download run16_best.pt"
        exit 1
    fi
fi
...
echo "  Resuming from: $RUN16_CKPT (run 16 best)"
echo "  Config: training/configs/segmentation_a100_run17.yaml"
echo ""

python -m training.train_segmentation \
    --config training/configs/segmentation_a100_run17.yaml \
    --reset-epochs \
    --resume "$RUN16_CKPT" \
...
if [ -f "checkpoints/segmentation/best.pt" ]; then
    cp checkpoints/segmentation/best.pt checkpoints/segmentation/run17_best.pt
    python -m training.export_onnx \
        --model segmentation \
        --checkpoint checkpoints/segmentation/run17_best.pt \
        --output ./models/onnx/segmentation_run17.onnx \
        2>&1 | tee "$LOG_DIR/export.log"
    echo "  Exported segmentation_run17.onnx"
fi

rclone copy ./checkpoints/segmentation/run17_best.pt \
    hetzner:strata-training-data/checkpoints_run17_seg/segmentation/ \
    --transfers 32 --fast-list -P
rclone copy "$LOG_DIR/" hetzner:strata-training-data/logs/run17_seg_${TIMESTAMP}/ \
    --transfers 32 --fast-list -P
if [ -f "./models/onnx/segmentation_run17.onnx" ]; then
    rclone copy ./models/onnx/segmentation_run17.onnx \
        hetzner:strata-training-data/models/onnx_run17_seg/ \
        --transfers 32 --fast-list -P
fi

echo ""
echo "============================================"
echo "  Run 17 Seg complete!"
echo "  Finished: $(date)"
echo "  Results:"
grep -E "New best mIoU|mIoU=" "$LOG_DIR/segmentation.log" 2>/dev/null | tail -5 || echo "  (check logs)"
echo ""
echo "  To download to Mac:"
echo "    rclone copy hetzner:strata-training-data/checkpoints_run17_seg/ /Volumes/TAMWoolff/data/checkpoints_run17_seg/ --transfers 32 --fast-list -P"
echo "    rclone copy hetzner:strata-training-data/models/onnx_run17_seg/ /Volumes/TAMWoolff/data/models/onnx_run17_seg/ --transfers 32 --fast-list -P"
echo "============================================"
