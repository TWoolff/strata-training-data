#!/usr/bin/env bash
# =============================================================================
# Strata Training — Back View Generation Run 5 (A100)
#
# PatchGAN adversarial loss + stronger augmentation.
# Resume from run 4 checkpoint (val/l1 ~0.215).
# Same data: ~3,049 triplets.
#
...
#
# Usage:
#   chmod +x training/run_back_view_run5.sh
#   ./training/run_back_view_run5.sh
# =============================================================================
set -euo pipefail

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_DIR="./logs/run5_back_view_${TIMESTAMP}"
mkdir -p "$LOG_DIR"

echo "============================================"
echo "  Strata Training — Back View Run 5 (GAN)"
echo "  Started: $(date)"
echo "  Logs: $LOG_DIR"
echo "============================================"
echo ""
...
if [ -f "$RUN4_CKPT" ]; then
    ...
    echo "  Downloading run 4 checkpoint..."
    ...
    if [ ! -f "$RUN4_CKPT" ]; then
        # Fallback: try latest best.pt
        rclone copy hetzner:strata-training-data/checkpoints_back_view_run4/best.pt \
            ./checkpoints/back_view/ --transfers 32 --fast-list -P
        if [ -f "checkpoints/back_view/best.pt" ]; then
            cp checkpoints/back_view/best.pt "$RUN4_CKPT"
        else
            echo "  WARN: Could not download run 4 checkpoint — training from scratch"
        fi
    fi
fi
# Place as latest.pt so auto-resume picks it up
if [ -f "$RUN4_CKPT" ] && [ ! -f "checkpoints/back_view/latest.pt" ]; then
    cp "$RUN4_CKPT" checkpoints/back_view/latest.pt
fi
echo ""
...
# 2. Train
# ---------------------------------------------------------------------------
echo "[2/4] Training back view model (with PatchGAN)..."
echo "  Config: training/configs/back_view_a100_run5.yaml"
echo ""

python3 -m training.train_back_view \
    --config training/configs/back_view_a100_run5.yaml \
...
if [ -f "checkpoints/back_view/best.pt" ]; then
    cp checkpoints/back_view/best.pt checkpoints/back_view/run5_best.pt
    python3 -m training.export_onnx \
        --model back_view \
        --checkpoint checkpoints/back_view/run5_best.pt \
        --output ./models/onnx/back_view_run5.onnx \
        2>&1 | tee "$LOG_DIR/export.log"
    ONNX_SIZE=$(du -h ./models/onnx/back_view_run5.onnx 2>/dev/null | cut -f1)
    echo "  Exported back_view_run5.onnx ($ONNX_SIZE)"
fi
echo ""
...
rclone copy ./checkpoints/back_view/run5_best.pt \
    hetzner:strata-training-data/checkpoints_back_view_run5/ \
    --transfers 32 --fast-list -P
rclone copy "$LOG_DIR/" hetzner:strata-training-data/logs/run5_back_view_${TIMESTAMP}/ \
    --transfers 32 --fast-list -P
if [ -f "./models/onnx/back_view_run5.onnx" ]; then
    rclone copy ./models/onnx/back_view_run5.onnx \
        hetzner:strata-training-data/models/back_view_run5/ \
        --transfers 32 --fast-list -P
fi

echo ""
echo "============================================"
echo "  Back View Run 5 complete!"
echo "  Finished: $(date)"
...
echo "    rclone copy hetzner:strata-training-data/checkpoints_back_view_run5/ /Volumes/TAMWoolff/data/checkpoints_back_view_run5/ --transfers 32 --fast-list -P"
echo "    rclone copy hetzner:strata-training-data/models/back_view_run5/ /Volumes/TAMWoolff/data/models/back_view_run5/ --transfers 32 --fast-list -P"
echo "============================================"
